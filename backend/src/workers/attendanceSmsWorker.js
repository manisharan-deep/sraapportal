...
const startAttendanceSmsWorker = () => {
  try {
    ...
    attendanceSmsQueue.on('error', (error) => {
      logger.error('Attendance SMS queue error', { error: error.message });
    });

    logger.info('Attendance SMS worker started');
  } catch (error) {
    logger.error('Attendance SMS worker failed to start', { error: error.message });
  }
};
